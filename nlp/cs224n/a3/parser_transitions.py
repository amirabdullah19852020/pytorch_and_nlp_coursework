...
class PartialParse(object):
    ...
    def __str__(self):
        return f"Stack is {self.stack} and buffer is {self.buffer}"
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            removed = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], removed))
        elif transition == 'RA':
...
def minibatch_parse(sentences, model, batch_size):
    ...
    dependencies = [[]] * len(unfinished_parses)
    unfinished_parses = list(filter(lambda x: not x.is_empty(), unfinished_parses))

    while len(unfinished_parses) > 0:
        current_batch = unfinished_parses[:batch_size]
        # print(f"We have {len(current_batch)} items remaining")
        # for i in range(len(current_batch)):
        #    print(f"Current item at index {i} is {current_batch[i]}")
        transitions = model.predict(current_batch)
        for index, transition in enumerate(transitions):
            current_batch[index].parse_step(transition)
