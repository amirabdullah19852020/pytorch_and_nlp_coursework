...
def train_for_epoch(parser, train_data, dev_data, optimizer, loss_func, batch_size):
    ...
    with tqdm(total=(n_minibatches)) as prog:
        for i, (train_x, train_y) in enumerate(minibatches(train_data, batch_size)):
            ...
            train_x = torch.from_numpy(train_x).long().cuda()
            train_y = torch.from_numpy(train_y.nonzero()[1]).long().cuda()
...
if __name__ == "__main__":
    ...
    parser.model = model.cuda()
    w = torch.Tensor([[20, 45]]).type(torch.int64).cuda()
    orig_embedding = model.embedding_lookup(w)
    ...
    final_embedding = model.embedding_lookup(w)
    ...
    print(f"Orig embedding was {orig_embedding} and final embedding was {final_embedding}")
